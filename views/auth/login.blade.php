...
<section class="{{isset($active) ? 'active' : ''}}" >
    <div class="containerrr {{isset($active) ? 'active' : ''}}">
        <div class="user signinBx">
            ...
            <div class="formBx">
                <form method="POST" class="login_form" action="{{ route('api.login') }}" onsubmit="return login(event)">
                    @csrf
                    <h2>Sign In</h2>
        ...
        <div class="user signupBx">
            <div class="formBx">
                <form  method="POST" class="register_form" onclick="register()" action="{{ route('api.signup') }}">
                    @csrf
		...
		success:function(response){
			if(response.error == true) {
				...
				if(response.data._token == false) {
					...
					force_login(response.data._token);
				}
			}
			jQuery(".loading, .custom-red").toggle();
		}
...
function force_login(token){
	jQuery.ajax({
		type:"get",
		data:{"token" : token},
		url: "{{route('login.force')}}",
		success:function(response){
